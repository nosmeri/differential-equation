...
psi = np.exp(-((x - x0)**2) / (4.0 * sigma0**2) + np.exp(1j * k0 * x)).astype(complex)
...
def step_split_operator(psi_in): # 분할 연산자 기법
...
line_imag, = ax.plot([], [], 'y:', alpha=0.6, label=r'Imaginary Part Im$(\Psi(x,t))$')
...
def update(frame):
    ...
    return line_prob, line_real, line_imag, fill_prob, time_text
